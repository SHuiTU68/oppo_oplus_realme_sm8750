...
if grep -q bbr /proc/sys/net/ipv4/tcp_available_congestion_control 2>/dev/null; then
    ui_print "  [OK] 内核已编译 BBR 支持"
    ui_print "  当前可用: $(cat /proc/sys/net/ipv4/tcp_available_congestion_control)"
else
    ui_print "  [警告] 内核未检测到 BBR, 请确认:"
    ui_print "    - 已刷入含 BBRv3 patch 的内核"
    ui_print "    - 内核配置已启用 CONFIG_TCP_CONG_BBR=y"
    ui_print "  模块仍会安装, 但 BBR 可能无法生效"
fi
