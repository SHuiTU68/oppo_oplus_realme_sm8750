...
integrate() {
  ...
  common_root="${1:-$PWD}"
  # 兼容上游 ZZH_PATCHES 布局(zram/zstdp/) 与本仓库布局(zram_patch/zstdp/)
  if [ -n "${ZZH_PATCHES:-}" ] && [ -d "${ZZH_PATCHES}/zram/zstdp" ]; then
    repo_root="$ZZH_PATCHES/zram"
  elif [ -n "${ZZH_PATCHES:-}" ] && [ -d "${ZZH_PATCHES}/zstdp" ]; then
    repo_root="$ZZH_PATCHES"
  else
  ...
  require_dir "$repo_root/zstdp"

  mkdir -p "$target_dir"
  copy_file "$repo_root/zstdp/Kconfig" "$target_dir/Kconfig"
  copy_file "$repo_root/zstdp/Makefile" "$target_dir/Makefile"
  copy_file "$repo_root/zstdp/zstdp_namespace.h" "$target_dir/zstdp_namespace.h"
  copy_file "$repo_root/zstdp/zstdp_wrapper.c" "$target_dir/zstdp_wrapper.c"
  if [ -f "$common_root/drivers/block/zram/backend_zstd.h" ]; then
    copy_file "$repo_root/zstdp/backend_zstdp.c" "$common_root/drivers/block/zram/backend_zstdp.c"
    copy_file "$repo_root/zstdp/backend_zstdp.h" "$common_root/drivers/block/zram/backend_zstdp.h"
  fi
  ...
  copy_file "$repo_root/zstdp/vendor_include_linux_unaligned.h" \
